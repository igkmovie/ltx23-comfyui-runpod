...
これら3ファイルはCivitaiのログイン/APIトークンが必要なため、
`models-manifest.json`のエントリは自動ダウンロード不可の`MANUAL:`プレースホルダに
なっています。**bootstrap実行前に、手動でダウンロードしたファイルを
`${COMFY_ROOT}/models/loras/`に配置してください**(例: `scp`でローカルから
RunPodの永続ボリュームへ直接転送)。ファイルが既に存在すればbootstrapの
ダウンロードステップはスキップされ、正常に進みます。未配置のまま実行すると、
`MANUAL:`文字列をURLとしてcurlしようとして明確なエラーで失敗します
(黙って壊れたファイルを保存することはありません)。
