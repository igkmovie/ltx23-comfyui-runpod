...
```
Lightning LoRA → Anime Style LoRA → 2D Animation Effects LoRA → (low側のみ)NSFW Motion Fix LoRA
```

`anime_style_wan22_i2v_low_noise.safetensors`はCivitaiの
[Anime Style [WAN 2.2 I2V] - Low Noise](https://civitai.com/models/2222779/anime-style-wan-22-i2v)
から取得する想定で、`models-manifest.json`は現状`TODO`プレースホルダのまま
です。ファイルを入手したら、上記2バリアント同様に(a) 手動で`models/loras/`
に置くか、(b) 公開バケットにアップロードしてmanifestのURLを差し替えるかを
選び、`models-manifest.json`の該当エントリを埋めてください。
