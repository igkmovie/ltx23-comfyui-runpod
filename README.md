...
low:  Lightning LoRA → Anime Style LoRA → 2D Animation Effects LoRA → NSFW Motion Fix LoRA
```
...
LoRAですが、配布ページの名前が示す通りlow-noise専用として作られているため、
NSFW Motion Fix LoRAと同様にhigh側には繋いでいません。他の2ファイル同様、
公開バケット(`creachat-trial-2026-wan22-loras-public`)経由でbootstrapが
自動ダウンロードします。
